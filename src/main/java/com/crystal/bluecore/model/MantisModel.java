...
import com.crystal.bluecore.entity.client.MantisAnimations;
...
public class MantisModel<T extends MantisEntity> extends SinglePartEntityModel<T> {
    /** 实体模型图层 */
    public static final EntityModelLayer MANTIS = new EntityModelLayer(Identifier.of(BlueCore.MOD_ID, "mantis"), "main");
    ...
    public MantisModel(ModelPart root) {
        ...
        mantis = this.root.getChild("mantis");
        head = mantis.getChild("head");
    }
    ...
    public void setAngles(MantisEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        // 重要代码：防止动画播放重复，与其他动画重叠现象
        getPart().traverse().forEach(ModelPart::resetTransform);
        setHeadAngles(netHeadYaw, headPitch);

        /* 行走动画 */
        animateMovement(MantisAnimations.ANIM_MANTIS_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        /* 空闲动画 */
        updateAnimation(entity.idleAnimationState, MantisAnimations.ANIM_MANTIS_IDLE, ageInTicks, 1f);
    }

    /**
     * <p>设置螳螂头部始终朝向玩家看</p>
     * @param headYaw 头部偏移
     * @param headPitch 头部顶点
     */
